...
BEGIN
  ...
  FOR ch_n IN bP..bR  LOOP        /* positionel data, centrum bedst */
     ...
     WHEN ch_n in (bK,bM) THEN
          ...
          END IF;
      WHEN ch_n in (bB,bQ) THEN  /* l�ber,D ikke specielt i centrum, men fianciettering */
          p := 630;
  ...
  ELSE
    p := 650;
  /* t�rne (rook) + Dronning (queen) p� (at) 7+8 r�kke (row) bonus */
    FOR n IN 1..8 LOOP
    ...
    /* b�nder (pawns) mere v�rd jo t�ttere de er p� forvandling (promotion) 7, 9 */
  ...
  /* konge (king) mere v�rd hvis i sikkerhed (secure) */
  IF not EndGame THEN
    p := 690;
    FOR m IN 1..2 LOOP
      FOR n IN 2..3 LOOP
...
--                WRT('bonus for moving king towards the white king to mate yx='||y||x);
 ...
 p := 450;
  ...
  IF (stilling(stOff+ 42)=bB) OR (stilling(stOff+ 42)=bN) THEN 
    ix := pdN(wP,31); pdw(ix) := pdw(ix) + pawnh3a3attack;--f�r 38, m� v�re en fejl
  END IF;
  IF (stilling(stOff+ 52)=wB) OR (stilling(stOff+ 52)=wN) THEN 
    ix := pdN(bP,61); pdw(ix) := pdw(ix) + pawnh3a3attack;--f�r 68, m� v�re en fejl
  END IF;
...
/* Is called thousands of times for each engine call! */
  ...
  inmv SIMPLE_INTEGER  := 0;   -- Positionel, matriel v�rdi
      ...
      FOR x IN 1..8 LOOP
        ...
        CASE 
         WHEN ch in (bP,bE) THEN 
                    IF KingEndGameB THEN
                      ...
                      FOR m_n in 2..y-2 LOOP --testet grund�gt!!!
                        m := m_n*10+x;
                        Bpawns(m)  :=TRUE;
                        Bpawns(m-1):=TRUE;
                        Bpawns(m+1):=TRUE;
        ...
        WHEN ch in (wP,wE)  THEN 
                    IF KingEndGameW THEN
                      ...
                      FOR m_n in 2+y..7 LOOP--testet grund�gt!!!
                        m := m_n*10+x;
                        Wpawns(m)  :=TRUE;
                        Wpawns(m-1):=TRUE;
                        Wpawns(m+1):=TRUE;
