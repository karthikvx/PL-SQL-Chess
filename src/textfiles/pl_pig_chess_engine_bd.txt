...
/*
  ValueT CONSTANT SIMPLE_INTEGER  := 470; --rook: danish: T�rn
  ValueR CONSTANT SIMPLE_INTEGER  := 480; --rook (castling possible) danish: t�rn (Rokade)
  ValueM CONSTANT SIMPLE_INTEGER  :=9999; --king (castling possible) danish: Konge (Majest�t)
  ValueK CONSTANT SIMPLE_INTEGER  :=9980; --king danish: Konge
  ValueD CONSTANT SIMPLE_INTEGER  := 880; --Queen danish: Dronning
  ValueL CONSTANT SIMPLE_INTEGER  := 300; --Bishop danish: L�ber
...
*/
  ...
  stDcount    CONSTANT SIMPLE_INTEGER :=8;-- f�r 0
...
BEGIN -- two-dimensional array (bP..wR, 11..88) -> 1..3978. Here index is calculated out of Numeric repr�sentation (ascii value) of pieces.
...
/*
  ...
  retn(brik,0]           indeholder f�rste retning.
  retn(brik,retning]     indeholder s� n�ste retning indtil den er 0.
...
-- finder s/h n�ste tr�k med nuv�rende brik, til=fra -> til=89 n�r ikke flere
-- !!!!!!!!!! slag for enpassant markeres IKKE for sort, b�r laves
  Done BOOLEAN;
...
-- udf�rer (Hvidt/Sort) tr�k, der i forvejen er valideret OK 
-- MoveTyp skal v�re defineret (og �ndres ikke her) 
  n SIMPLE_INTEGER :=0;
BEGIN
  IF stilling(stOff+ HvisTur)=wT THEN
    CASE 
      ...
      WHEN stilling(stOff+ fra) = wC  THEN 
               stilling(stOff+ til):=wR;        -- fjern t�rns rokaderet 
               -- hvis ogs� andet t�rn har v�ret flyttet s� fjern konges rokaderet 
    ...
    ELSIF SET_IN(MOVErokade,MoveTyp) THEN
      IF til<fra THEN             -- move/convert rook (t�rn) 
  ...
  ELSE
    CASE
      ...
      WHEN stilling(stOff+ fra) = bC  THEN 
               stilling(stOff+ til):=bR;        -- remove rook (t�rns) castling right
      ...
      WHEN stilling(stOff+ fra) = bM  THEN 
               stilling(stOff+ til):=bK;        -- remove King (t�rns) castling right
...
-- udf�rer (Hvidt/Sort) tr�k, der i forvejen er valideret OK
-- som DoMove, men kr�ver IKKE MoveTyp information
-- Hurtigere end DoMoveOK n�r tr�k ER valideret OK
...
BEGIN
  IF hvid THEN -- White check
    ...
    retning:=retn(rxOff+ wR) (ryOff+ 0);       -- look in all rook (t�rn) directions
    LOOP
      tst:=n;
      cnt:=0;
    ...
    END LOOP;
    IF NOT skak THEN
      retning:=retn(rxOff+ wB) (ryOff+ 0);       -- look in all bishop (l�ber) directions
      LOOP
        tst:=n;
        cnt:=0;
  ...
  ELSE -- black check
    ...
    retning:=retn(rxOff+ wR) (ryOff+ 0);       -- led i alle t�rn retninger 
    LOOP
      tst:=n;
      cnt:=0;
      LOOP                    -- find f�rste ikke-tomme felt i retningen 
        tst:=tst+retning;
        cnt := cnt + 1;
        EXIT WHEN stilling(stOff+ tst)<>spc;
       END LOOP;
    ...
    END LOOP;
    IF NOT skak THEN
      retning:=retn(rxOff+ wB) (ryOff+ 0);       -- led i alle l�ber retninger 
      LOOP
        tst:=n;
        cnt:=0;
        LOOP                    -- find f�rste ikke-tomme felt i retningen 
...
-- checker om (hvidt/sort) tr�k er ulovligt p.gr.a. at kongen er skak 
-- eller rokerer fra/over truet felt 
-- kr�ver at MoveTyp er sat korrekt n�r rokade og pat 
  skak BOOLEAN;
  n SIMPLE_INTEGER:=0;
...
PROCEDURE GetNext(stilling in out PL_PIG_CHESS_ENGINE_EVAL.STILLINGTYPE, 
                  ...
                  MoveTyp in out MOVETYPE) IS
-- finder n�ste (hvide/sorte) tr�k i stillingen, fra=89 n�r ikke flere
  hvid BOOLEAN;
BEGIN
  IF stilling(stOff+ HvisTur)=wT THEN
    LOOP
...
-- fuldst�ndigt check af om (s/h) tr�k er ok, genererer sort/hvide tr�k med DoMove
-- IF not DoMoveOk() then fejl.. -> udf�rer et tr�k hvis det er lovligt
  fr SIMPLE_INTEGER:=0;
  ti SIMPLE_INTEGER:=0;
  retning SIMPLE_INTEGER:=0;
  black BOOLEAN; 
        ...
        IF SET_IN (MOVEslag,movetyp) THEN
          ...
          IF n<MinEgain THEN
            ...
            IF m>0 THEN
              ...
              END IF;
            ELSIF (SET_IN (MOVEskak , movetyp)) AND (Qdepth<2) THEN -- 1: se p� ofre med skak
              n:=1;
            END IF;
          END IF;
...
--WRITELN(s('KING=')+l(KING));*)
  ...
  countt:=0;
  reps:=0;
  LOOP           -- lav liste med alle tr�k (count)
    GetNext(stilling,fr,ti,retning,movetyp);
    IF (fr<89) THEN
      countt := countt + 1;
      IF SET_IN (MOVEslag ,movetyp) THEN
...
--$ ENDIF
   ...
   -- Check om varianttr� har et ok forslag !!!!!!!!!!!!!!!!!!!!!!*)
...
-- 1'ste tr�k =1, Udf�rer tr�k hvis OK (fra<89)
